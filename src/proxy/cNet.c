...
*/
	...
	//启动 后端 读写 工作线程，启动前端读工作线程 和 前端的 主动链接线程
...
int dtShmSpace(int l_shm_id)
{
	if(l_shm_id > 0)
	{
		if(shmctl(l_shm_id, IPC_RMID, 0) < 0)
		{
			swVdebug(ERROR, "S3101.15: 删除已经存在的共享内存...失败...失败..errno==[%s]", strerror(errno));
			return P_FAILRE;
		}
	}
    return P_SUCCESS;
...
int  initMemPool()
{
	//内存池存放 tree 的  data 节点。
	//内存池 需要 加锁 啊
	////内存池	 REQ   

	REQ_MEM_NODE *pReqTmp = NULL;
	RSP_MEM_NODE *pRspTmp = NULL;
	int iReqTmp = 0, iRspTmp = 0;

	memset(&gReqMemPool, 0x00, sizeof(MEM_POOL));
	gReqMemPool.maxPoolNode = M_REQ_POOL_NODE_MAX;
	gReqMemPool.currentAvaliable = 0;
	gReqMemPool.sizeofNode = sizeof(REQ_MEM_NODE);
	pthread_mutex_init(&(gReqMemPool.mutex), NULL);
	gReqMemPool.pool = NULL;
	gReqMemPool.pool = malloc(gReqMemPool.maxPoolNode*gReqMemPool.sizeofNode);
	if(gReqMemPool.pool == NULL)
	{
		showmsg_pth(ERROR,"开辟内存池错误 gReqMemPool.pool == NULL error==[%s]\n", strerror(errno));
		return X_FAILRE;
	}
	memset((unsigned char*)gReqMemPool.pool, 0x00, gReqMemPool.maxPoolNode*gReqMemPool.sizeofNode);
	pReqTmp = (REQ_MEM_NODE*)gReqMemPool.pool;
	iReqTmp = gReqMemPool.currentAvaliable;
	while(iReqTmp != M_REQ_POOL_NODE_MAX)
	{
		pReqTmp->useredFlag = MEM_POOL_NODE_UNUNSED;
		pReqTmp->req.fd_status = SOCK_FD_INIT_READ;
		pReqTmp+=iReqTmp;
		iReqTmp++;
	}


	memset(&gRspMemPool, 0x00, sizeof(MEM_POOL));
	gRspMemPool.maxPoolNode = M_RSP_POOL_NODE_MAX;
	gRspMemPool.currentAvaliable = 0;	
	gRspMemPool.sizeofNode = sizeof(RSP_MEM_NODE);
	pthread_mutex_init(&(gRspMemPool.mutex), NULL);
	gRspMemPool.pool = NULL;
	gRspMemPool.pool = malloc(gRspMemPool.maxPoolNode*gRspMemPool.sizeofNode);
	if(gRspMemPool.pool == NULL)
	{
		showmsg_pth(ERROR,"开辟内存池错误 gRspMemPool.pool == NULL error==[%s]\n", strerror(errno));
		return X_FAILRE;
	}
	memset((unsigned char*)gRspMemPool.pool, 0x00, gRspMemPool.maxPoolNode*gRspMemPool.sizeofNode);
	pRspTmp = (RSP_MEM_NODE*)gRspMemPool.pool;
	iRspTmp = gRspMemPool.currentAvaliable;
	while(iRspTmp != M_REQ_POOL_NODE_MAX)
	{
		pRspTmp->useredFlag = MEM_POOL_NODE_UNUNSED;
		pRspTmp+=iRspTmp;
		iRspTmp++;
	}	
	return X_SUCCESS;
...
int getTreeNodeFromMemPool(int  type, ...)
{
	MEM_POOL *memPool = NULL;
	unsigned char **pNodeMem = NULL;
	REQ_MEM_NODE *req = NULL;
	RSP_MEM_NODE *rsp = NULL;
	
	va_list ap;
	va_start(ap, type);
	pNodeMem = va_arg(ap, unsigned char**);
	va_end(ap);
	
	if(type==M_REQ_MEM_POOL)
	{
		memPool = &gReqMemPool;
		req = (REQ_MEM_NODE*)(memPool->pool);
		pthread_mutex_lock(&(memPool->mutex));
		if(memPool->currentAvaliable<=0)
		{
			pthread_mutex_unlock(&(memPool->mutex));
			showmsg_pth(ERROR,"currentAvaliable<=0 没有可用内存节点 TYPE==[%d]\n", M_RSP_MEM_POOL);
			return X_FAILRE;
		}
		while(req <= ((REQ_MEM_NODE*)(memPool->pool) + memPool->maxPoolNode))
		{
			if(req->useredFlag == MEM_POOL_NODE_UNUNSED)
			{
				req->useredFlag = MEM_POOL_NODE_USED;
				memPool->currentAvaliable--;
				*pNodeMem = (unsigned char*)req;
				pthread_mutex_unlock(&(memPool->mutex));
				showmsg_pth(DEBUG,"从内存池MEM_POOL_NODE_UNUNSED获取节点成功[%d]\n", req->useredFlag);
				return X_SUCCESS;
			}
			req++;
		}
		pthread_mutex_unlock(&(memPool->mutex));
		showmsg_pth(ERROR,"从内存池M_RSP_MEM_POOL获取节点 失败  建议回收，扩大内存===[%d]\n", strerror(errno));
		return X_FAILRE;
	}
	else if(type==M_RSP_MEM_POOL)
	{
		memPool = &gRspMemPool;
		rsp = (RSP_MEM_NODE*)(memPool->pool);
		pthread_mutex_lock(&(memPool->mutex));
		if(memPool->currentAvaliable<=0)
		{
			pthread_mutex_unlock(&(memPool->mutex));
		...
		while(rsp <= ((RSP_MEM_NODE*)(memPool->pool) + memPool->maxPoolNode))
		{
			if(rsp->useredFlag == MEM_POOL_NODE_UNUNSED)
			{
				rsp->useredFlag = MEM_POOL_NODE_USED;
				memPool->currentAvaliable--;
				*pNodeMem = (unsigned char*)rsp;
				pthread_mutex_unlock(&(memPool->mutex));
				showmsg_pth(DEBUG,"从内存池MEM_POOL_NODE_UNUNSED获取节点成功[%d]\n", rsp->useredFlag);
				return X_SUCCESS;
			}
		...
		showmsg_pth(ERROR,"type 既不是 REQ	也不是 RSP	传递参数有误[%d]\n", type);
	pthread_mutex_unlock(&(memPool->mutex));
	showmsg_pth(ERROR,"从内存池获取节点 失败  建议回收，扩大内存===[%d]\n", strerror(errno));

	return X_FAILRE;
}

int putTreeNodeToMemPool(int  type, ...)
{
	MEM_POOL *memPool = NULL;
	unsigned char *pNodeMem = NULL;
	REQ_MEM_NODE *req = NULL;
	...
	if(type==M_REQ_MEM_POOL)
	{
		...
		req = (REQ_MEM_NODE*)(memPool->pool);
		pthread_mutex_lock(&(memPool->mutex));
		if(memPool->currentAvaliable >= memPool->maxPoolNode)
		{
			pthread_mutex_unlock(&(memPool->mutex));
			showmsg_pth(ERROR,"currentAvaliable >= maxPoolNode 都已经回收完毕  还有释放需求,就是出错了 TYPE==[%d]\n", M_REQ_MEM_POOL);
			return X_FAILRE;
		}
		memset(req, 0x00, sizeof(REQ_MEM_NODE));
		req->useredFlag = MEM_POOL_NODE_UNUNSED;
		memPool->currentAvaliable++;
		pthread_mutex_unlock(&(memPool->mutex));		
		showmsg_pth(DEBUG,"释放内存节点到内存池 成功===[%d]\n", req->useredFlag);
		return X_SUCCESS;
	}
	else if(type==M_RSP_MEM_POOL)
	{
		...
		memset(rsp, 0x00, sizeof(REQ_MEM_NODE));
		rsp->useredFlag = MEM_POOL_NODE_UNUNSED;
		memPool->currentAvaliable++;
		pthread_mutex_unlock(&(memPool->mutex));		
		showmsg_pth(DEBUG,"释放内存节点到内存池 成功===[%d]\n", rsp->useredFlag);
		return X_SUCCESS;
	...
	showmsg_pth(ERROR,"释放内存节点到内存池失败[%d]\n", type);
	return X_FAILRE;
}



//TODO  这里应该加上锁，，，，，，，，，，，，，，，，，，，，，，20190411 jll
int reqRbTreeInsert(struct rb_root *reqTree, REQ_TREE_NODE  *data)
{
  struct rb_node **new = &(reqTree->rb_node), *parent = NULL;

  /* Figure out where to put new node */
  while (*new) {
	  REQ_TREE_NODE *this = container_of(*new, REQ_TREE_NODE, node);

	  //TODO  20190425 JLL  这里 后续 需要 修改
	  int result = memcmp(data->userData, this->userData, 32);
  ...
  rb_insert_color(&data->node, reqTree);

  return 0;
}

int rspRbTreeInsert(struct rb_root *rspTree, RSP_TREE_NODE  *data)
{
  struct rb_node **new = &(rspTree->rb_node), *parent = NULL;

  /* Figure out where to put new node */
  while (*new) {
	  RSP_TREE_NODE *this = container_of(*new, RSP_TREE_NODE, node);
  ...
  /* Add new node and rebalance tree. */
  rb_link_node(&data->node, parent, new);
  rb_insert_color(&data->node, rspTree);

  return 0;
}

REQ_TREE_NODE *ReqRbTreeSearch(struct rb_root *reqTree, char *string)
{
  struct rb_node *node = reqTree->rb_node;

  while (node) {
	  REQ_TREE_NODE *data = container_of(node, REQ_TREE_NODE, node);
	  int result;

	  result = memcmp(string, data->userData, 32);
...
RSP_TREE_NODE *RspRbTreeSearch(struct rb_root *rspTree, char *string)
{
  struct rb_node *node = rspTree->rb_node;

  while (node) {
	  RSP_TREE_NODE *data = container_of(node, RSP_TREE_NODE, node);
	  int result;
...
void ReqRbTreeDelete(struct rb_root *reqTree, char *key_string)
{
	REQ_TREE_NODE *data = ReqRbTreeSearch(reqTree, key_string);

	if(data)
	{
		rb_erase(&data->node, reqTree);	 //释放 node 
		//myfree(data);//这里要 释放  key  和 data	 考虑一下  data要不要 维护 malloc 内存池
	}
}

void RspRbTreeDelete(struct rb_root *rspTree, char *key_string)
{
	RSP_TREE_NODE *data = RspRbTreeSearch(rspTree, key_string);

	if(data)
	{
		rb_erase(&data->node, rspTree);	 //释放 node 
		//myfree(data);//这里要 释放  key  和 data	 考虑一下  data要不要 维护 malloc 内存池
	}
}
